...
sdk: static
pinned: false
short_description: Create songs with DJ Reachy
tags:
 - reachy_mini
...
Set `REACHY_MINI_CUSTOM_PROFILE=<name>` to load `src/dj_reachy/profiles/<name>/` (see `.env.example`). If unset, the `default` profile is used.
...
```
Each placeholder pulls the matching file under `src/dj_reachy/prompts/` (nested paths allowed). See `src/dj_reachy/profiles/example/` for a reference layout.
...
sweep_look
```
Tools are resolved first from Python files in the profile folder (custom tools), then from the shared library `src/dj_reachy/tools/` (e.g., `dance`, `head_tracking`). 
...
Custom tools must subclass `dj_reachy.tools.core_tools.Tool` (see `profiles/example/sweep_look.py`).

### Edit personalities from the UI
When running with `--gradio`, open the “Personality” accordion:
- Select among available profiles (folders under `src/dj_reachy/profiles/`) or the built‑in default.
