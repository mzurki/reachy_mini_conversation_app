...
from dj_reachy.config import config  # noqa: F401


logger = logging.getLogger(__name__)


PROFILES_DIRECTORY = "dj_reachy.profiles"
...
class ToolDependencies:
    ...
    openai_realtime_handler: Any | None = None  # OpenaiRealtimeHandler for audio output queue
    elevenlabs_api_key: str | None = None  # ElevenLabs API key for music generation
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            try:
                shared_tool_module = f"dj_reachy.tools.{tool_name}"
