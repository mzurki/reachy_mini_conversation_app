...
class MovementState:
    ...
    # Music dance offsets: head (x,y,z,roll,pitch,yaw), antennas (l,r), body_yaw
    music_dance_head_offsets: Tuple[float, float, float, float, float, float] = (
        0.0,
        0.0,
        0.0,
        0.0,
    ...
    music_dance_antennas: Tuple[float, float] = (0.0, 0.0)
    music_dance_body_yaw: float = 0.0
...
class MovementManager:
    ...
    def __init__(
        ...
        # Music dance offsets (full body: head, antennas, body_yaw)
        self._music_dance_lock = threading.Lock()
        self._pending_music_dance_head: Tuple[float, float, float, float, float, float] = (
            0.0,
            0.0,
            0.0,
            0.0,
        ...
        self._pending_music_dance_antennas: Tuple[float, float] = (0.0, 0.0)
        self._pending_music_dance_body_yaw: float = 0.0
        self._music_dance_dirty = False
    ...
    def set_music_dance_offsets(
        self,
        head_offsets: Tuple[float, float, float, float, float, float],
        antennas: Tuple[float, float],
        body_yaw: float,
    ) -> None:
        """Update music-reactive dance offsets for full body movement.

        Args:
            head_offsets: (x, y, z, roll, pitch, yaw) in metres/radians (world frame)
            antennas: (left, right) antenna positions in radians
            body_yaw: Body yaw offset in radians

        Thread-safe via a pending snapshot.
        """
        with self._music_dance_lock:
            self._pending_music_dance_head = head_offsets
            self._pending_music_dance_antennas = antennas
            self._pending_music_dance_body_yaw = body_yaw
            self._music_dance_dirty = True
    ...
    def _apply_pending_offsets(self) -> None:
        """Apply the most recent speech/face/music-dance offset updates."""
        ...
        # Music dance offsets (full body)
        music_head: Tuple[float, float, float, float, float, float] | None = None
        music_antennas: Tuple[float, float] | None = None
        music_body_yaw: float | None = None
        with self._music_dance_lock:
            if self._music_dance_dirty:
                music_head = self._pending_music_dance_head
                music_antennas = self._pending_music_dance_antennas
                music_body_yaw = self._pending_music_dance_body_yaw
                self._music_dance_dirty = False

        if music_head is not None:
            self.state.music_dance_head_offsets = music_head
            self.state.music_dance_antennas = music_antennas  # type: ignore
            self.state.music_dance_body_yaw = music_body_yaw  # type: ignore
            self.state.update_activity()
    ...
    def _get_secondary_pose(self) -> FullBodyPose:
        """Get the secondary full body pose from speech, face tracking, and music dance offsets."""
        # Combine speech sway offsets + face tracking offsets + music dance head offsets
        secondary_offsets = [
            self.state.speech_offsets[0] + self.state.face_tracking_offsets[0] + self.state.music_dance_head_offsets[0],
            self.state.speech_offsets[1] + self.state.face_tracking_offsets[1] + self.state.music_dance_head_offsets[1],
            self.state.speech_offsets[2] + self.state.face_tracking_offsets[2] + self.state.music_dance_head_offsets[2],
            self.state.speech_offsets[3] + self.state.face_tracking_offsets[3] + self.state.music_dance_head_offsets[3],
            self.state.speech_offsets[4] + self.state.face_tracking_offsets[4] + self.state.music_dance_head_offsets[4],
            self.state.speech_offsets[5] + self.state.face_tracking_offsets[5] + self.state.music_dance_head_offsets[5],
        ]
        ...
        # Include music dance antennas and body_yaw as secondary offsets
        secondary_antennas = self.state.music_dance_antennas
        secondary_body_yaw = self.state.music_dance_body_yaw

        return (secondary_head_pose, secondary_antennas, secondary_body_yaw)
